...
int main(void)
{
    ...
    for (int i = 0; i < 7; ++i)
    {
        strand->post(boost::bind(&Print, i));
        std::cout << "[ POST ] " << i << std::endl;
    }

    strand->post(boost::bind(&Print, 10));
    strand->post(boost::bind(&Print, 20));
    strand->post(boost::bind(&Print, 30));
    strand->post(boost::bind(&Print, 40));
    strand->post(boost::bind(&Print, 50));

    int i = 0;
    strand->post(boost::bind(&Print2, boost::ref(i)));
