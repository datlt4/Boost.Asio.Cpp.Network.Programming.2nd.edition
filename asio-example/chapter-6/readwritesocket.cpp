...
void ClientContext::Close()
{
    ...
    m_socket.close(ec);
    std::cout << "[ m_socket.close ]" << std::endl;
}

void ClientContext::OnSend(const boost::system::error_code &ec, std::list<std::vector<boost::uint8_t>>::iterator itr)
{
    if (ec)
    {
        global_stream_lock.lock();
...
void ClientContext::Send(const void *buffer, size_t length)
{
...
void ClientContext::OnRecv(const boost::system::error_code &ec, size_t bytes_transferred)
{
    if (ec)
    {
        global_stream_lock.lock();
